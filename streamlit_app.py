...
def generate_html_report(df, patient_name):
    ...
    for sno, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        rows_html.append(
            f'<tr{low}><td>{sno}</td>'
            f'<td class="gene-cell">{html_lib.escape(gene)}</td>'
            f'<td class="pct-cell">{pct_str}</td></tr>'
        )
...
<head>
  ...
  <style>
    *{{box-sizing:border-box;margin:0;padding:0}}
    body{{background:#fff;font-family:Arial,Helvetica,sans-serif;color:#333;font-size:14px}}
    .header{{
      display:flex;align-items:center;gap:16px;
      padding:10px 20px;border-bottom:1px solid #ddd;
    }}
    .header img{{height:65px;width:auto}}
    .header h1{{font-size:20px;font-weight:bold;color:#222}}
    .sample-bar{{
      padding:6px 20px;background:#f7f7f7;border-bottom:1px solid #ddd;
    ...
    .controls input{{
      padding:6px 12px;border:1px solid #bbb;border-radius:3px;
      font-size:13px;width:220px;outline:none;
    }}
    .controls input:focus{{border-color:#e04e2a}}
    .controls button{{
      padding:6px 14px;background:#e04e2a;color:#fff;
      border:none;border-radius:3px;cursor:pointer;font-size:13px;
    }}
    .controls button:hover{{background:#c43d1b}}
    .controls button.active{{background:#8b1a00}}
    .table-wrap{{overflow-y:auto;max-height:calc(100vh - 200px)}}
    table{{width:100%;border-collapse:collapse}}
    thead th{{
      background:#e04e2a;color:#fff;padding:10px 12px;
      text-align:center;font-size:13px;position:sticky;top:0;
    }}
    tbody tr{{border-bottom:1px solid #e0e0e0}}
    tbody tr:hover{{background:#f5f5f5}}
    tbody td{{padding:7px 12px;text-align:center;font-size:13px}}
    .gene-cell{{color:#1155cc}}
    .pct-cell{{color:#1155cc}}
    .low-row{{background:#fff0ee}}
    .low-row .gene-cell,.low-row .pct-cell{{color:#cc0000}}
    .low-row:hover{{background:#ffe4e0}}
  </style>
</head>
<body>
  <div class="header">
    <img src="data:image/png;base64,{LOGO_B64}" alt="Anderson Diagnostics &amp; Labs">
    <h1>Gene Coverage Metrics</h1>
  </div>
  <div class="sample-bar">Sample: {safe_patient_html}</div>
  <div class="controls">
    <input id="searchInput" type="text" placeholder="Search by Gene...">
  ...
  <div class="table-wrap">
    <table>
      <thead><tr><th>S.No</th><th>Gene</th><th>Percentage of coding region covered</th></tr></thead>
  ...
  <script>
    ...
    function applyFilters() {{
      ...
      rows.forEach(r => {{
        ...
        const vis = matchQ && (!showLowOnly || isLow);
        ...
        with col_gen:
            if st.button("🌐 Build HTML Report", use_container_width=True, type="primary",
                ...
                with st.spinner("Building report…"):
                    # Use ALL coverage genes (not just panel-filtered), sort alphabetically
                    cov = st.session_state.coverage_data.copy()
                    gene_col_c = next((c for c in ['Gene_ID','Gene_Name','Gene'] if c in cov.columns), cov.columns[0])
                    cov[gene_col_c] = cov[gene_col_c].astype(str).str.strip().str.upper()
                    cov = cov.sort_values(gene_col_c)
                    if has_mito_p:
                        mito_s = st.session_state.mito_data.sort_values('Gene_ID').rename(columns={'Gene_ID': gene_col_c})
                        df_for_html = pd.concat([cov, mito_s], ignore_index=True).drop_duplicates(subset=gene_col_c, keep='first')
                    else:
                        df_for_html = cov
